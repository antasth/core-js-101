...
function encodeToRot13(str) {
  ...
  return str.split('').map((symbol) => {
    const index = alphabet.indexOf(symbol.toLowerCase());
    let symbolCase = false;
    if (symbol === symbol.toLowerCase()) {
      symbolCase = true;
    } else symbolCase = false;

    if (symbolCase) {
      return alphabet.includes(symbol.toLowerCase())
